...
import DataMapping.DataForwardShortMessageToOnAccount;
 ...
 * this test case is used for checking if an account forward a message to an account and account receive the message successfully or not.
 * Pre-codition : account A on device 01 send a message to a account B (device 02)
 * Then account B will forward a massage to account A (device 01)
 * Verify account B forward message to account A successfully.
 * Verify account A receive forwarded message successfully.
 * The step detail is:
 ...
 * Step 3 : account B on device 02 forward a message to account A on device 01. Verify account B forward message successfully. Function "forwardShortMessageToAnAccount"
 * Step 4 : open app on device 01 to check account A receive message or not. Function "openAppOnDevice01ToCheckReceiveForwardMessage".
 * Step 5 : check if account A receive forward message from account B successfully. Function "checkReceiveForwardMessageSuccessfully".
 */


public class TestForwardMessageToOneAccount {
    AndroidDriver driver;
    AppiumServer appiumSrv;

    SetUpPhoneProfile phoneProfileSentObj;
    SetUpPhoneProfile phoneProfileReceiverObj;
    DataForwardShortMessageToOnAccount dataSendForward;
    ...
    final static Logger logger= Logger.getLogger(TestForwardMessageToOneAccount.class);
    ...
    /**
     * this function is used for forwarding a massage from account B (on device 02) to account A (on device 01).
     */
    @Test(priority = 1,groups="forwardMessageToAnAccount")
    public void forwardShortMessageToAnAccount(){
        // select account name which send the message
        scrHome.selectContact(accountSentName);


        // refresh screen chatting
        ...
        // select contact to forward message
        scrHome.selectContact(accountSentName);
    ...
    /**
     * this function is used for open app on device 01 to check if account A receive message
     * @throws IOException
     */
    @AfterGroups(groups ="forwardMessageToAnAccount" )
    public void openAppOnDevice01ToCheckReceiveForwardMessage() throws IOException {
    ...
    /**
     * this function is used for checking if account A receive message successfully.
     */
    @Test(priority = 2)
    public void checkReceiveForwardMessageSuccessfully(){
        // select account which receive forward message
        scrHome.selectContact(accountForwardName);
    ...
    public void sendMessage() throws IOException {
        ...
        dataSendForward= new DataForwardShortMessageToOnAccount(fileName);

        //Get the account name who receive message.
        //and get the content of message which will be sent.
        accountForwardName = dataSendForward.getAccountForward();
